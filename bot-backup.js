...
function createBuyKeyboard() {
    return Keyboard.inlineKeyboard([
        [
            ...
            Keyboard.button.callback('🎫 Разовый (700 руб.)', 'buy_single_select')
        ]
    ]);
}
        ...
        if (userId && responseType === 'yes') {
            try {
                ...
                if (subscriptionStatus === 'valid') {
                    ...
                    message += `***🎯 ЧТО ДЕЛАТЬ:***\n`;
                    message += `1. **Приходите на тренировку**\n`;
                    message += `2. **Оплатите разовое посещение** (700 руб.)\n`;
                    message += `3. **Или купите абонемент заранее**\n\n`;
                }
                ...
                if (subscriptionStatus !== 'valid') {
                    const buyKeyboard = Keyboard.inlineKeyboard([
                        [
                            Keyboard.button.callback('📅 Купить абонемент', 'user_panel_buy')
                        ]
                    ]);
                    
                    message += `***💳 КУПИТЬ АБОНЕМЕНТ:***\n`;
    ...
    try {
        ...
        const keyboard = Keyboard.inlineKeyboard([
            ...
            [
                ...
                Keyboard.button.callback('💳 Купить абонемент', 'user_buy')
            ],
            [
        ...
        await ctx.reply(
            ...
            `***💳 АБОНЕМЕНТ:***\n` +
            `${subscription ? 
                `└─ ✅ ${subscription.type === 'monthly' ? '📅 Месячный' : '🎫 Разовый'}\n` +
...
bot.command('моя_статистика', async (ctx) => {
    try {
        ...
        if (subscription) {
            ...
            statsText += `***🧾 АБОНЕМЕНТ:***\n`;
            statsText += `Тип: ${subscription.type === 'monthly' ? '📅 Месячный' : '🎫 Разовый'}\n`;
...
bot.command('абонемент', async (ctx) => {
    try {
        ...
        if (subscription) {
            ...
            response += `📅 Тип: ${subscription.type === 'monthly' ? '📅 Месячный' : '🎫 Разовый'}\n`;
            ...
            response += '🎫 Разовый (1 занятие) - 700 руб.\n\n';
...
bot.action('user_panel_buy', async (ctx) => {
    try {
        ...
        let response = `***💳 ПОКУПКА АБОНЕМЕНТА***\n\n`;
        ...
        response += `***🎫 РАЗОВЫЙ АБОНЕМЕНТ***\n`;
        response += `└─ 🎫 1 занятие\n`;
        response += `└─ 💰 Цена: 700 руб.\n`;
        response += `└─ ⏰ Неограниченный срок\n`;
        response += `└─ 🎯 Для пробного посещения\n\n`;
    ...
    try {
        ...
        const keyboard = Keyboard.inlineKeyboard([
            [
                ...
                Keyboard.button.callback('🎫 Купить разовый', 'user_buy_single')
            ],
            [
...
bot.action('user_subs_history', async (ctx) => {
    try {
        ...
        let response = `***📋 ИСТОРИЯ ПОКУПОК АБОНЕМЕНТОВ***\n\n`;
        
        if (!stats || !stats.subscriptionHistory || stats.subscriptionHistory.length === 0) {
            response += `📭 ***История покупок пуста***\n\n`;
            response += `У вас еще не было абонементов.\n`;
            response += `Приобретите первый абонемент!`;
        ...
        sortedHistory.forEach((sub, index) => {
            ...
            response += `   📋 Тип: ${sub.type === 'monthly' ? '📅 Месячный' : '🎫 Разовый'}\n`;
        ...
        response += `📅 Последняя покупка: ${new Date(sortedHistory[0].date).toLocaleDateString('ru-RU')}\n`;
...
bot.action('user_buy_compare', async (ctx) => {
    try {
        ...
        response += `└─ 🎫 ***Разовый*** - если ходите 1 раз в неделю\n`;
...
bot.action('user_buy_single', async (ctx) => {
    try {
        ...
        await ctx.reply(
            ...
            `1. Сообщите администратору\n` +
            `2. Абонемент будет активирован\n` +
            `3. Можно записаться на тренировку!\n\n` +
            
            `***Вы подтверждаете покупку разового абонемента?***`,
            {
                format: 'markdown',
                attachments: [keyboard]
            }
...
bot.action('confirm_buy_single', async (ctx) => {
    try {
        ...
        const response = `***✅ АБОНЕМЕНТ УСПЕШНО ОФОРМЛЕН!***\n\n` +
            ...
            `2. Если понравится - купите месячный абонемент\n` +
            `3. Начните регулярно тренироваться\n` +
            `4. Достигайте своих целей!\n\n` +
        ...
        logToFile(`✅ Пользователь ${userName} (${userId}) купил разовый абонемент`);
...
bot.action('buy_monthly_select', async (ctx) => {
    try {
        ...
        await ctx.reply(
            `***📅 ВЫБОР СПОСОБА ОПЛАТЫ ДЛЯ МЕСЯЧНОГО АБОНЕМЕНТА***\n\n` +
            `***📋 ДЕТАЛИ ЗАКАЗА:***\n` +
            `└─ Тип: Месячный абонемент\n` +
            `└─ Занятий: 8\n` +
            `└─ Срок: 30 дней\n` +
...
bot.action('buy_single_select', async (ctx) => {
    try {
        ...
        await ctx.reply(
            `***🎫 ВЫБОР СПОСОБА ОПЛАТЫ ДЛЯ РАЗОВОГО АБОНЕМЕНТА***\n\n` +
            `***📋 ДЕТАЛИ ЗАКАЗА:***\n` +
            `└─ Тип: Разовый абонемент\n` +
            `└─ Занятий: 1\n` +
            `└─ Срок: Неограниченный\n` +
...
bot.action(/^pay_cash_(monthly|single)$/, async (ctx) => {
    try {
        ...
        const subscriptionName = subscriptionType === 'monthly' ? 'Месячный' : 'Разовый';
        ...
        await ctx.reply(
            `***💰 ОПЛАТА НАЛИЧНЫМИ***\n\n` +
            `***📋 ВАШ ЗАКАЗ:***\n` +
            `└─ Тип: ${subscriptionName} абонемент\n` +
            `└─ Занятий: ${lessons}\n` +
            `└─ Сумма к оплате: ***${amount} руб.***\n\n` +
            
            `***📝 ИНСТРУКЦИЯ ПО ОПЛАТЕ:***\n` +
            `1. **Приходите на тренировку**\n` +
            `2. **Подойдите к администратору**\n` +
            `3. **Сообщите:** "Я оплачиваю абонемент"\n` +
            `4. **Назовите свое имя:** ${userName}\n` +
            `5. **Оплатите ${amount} руб.** наличными\n` +
            `6. **После оплаты** нажмите кнопку ниже\n\n` +
            ...
            `Администратор обычно присутствует за 30 минут до начала\n\n` +
            
            `***⚠️ ВНИМАНИЕ:***\n` +
            `Абонемент будет активирован только после подтверждения оплаты администратором\n` +
...
bot.action(/^pay_bank_(monthly|single)$/, async (ctx) => {
    try {
        ...
        await ctx.reply(
            ...
            `4. **Сохраните чек** об оплате\n` +
            `5. **Отправьте чек** администратору\n` +
            `6. **Абонемент активируется** в течение 15 минут\n\n` +
            
            `***🎯 ВЫБЕРИТЕ СПОСОБ ПЕРЕВОДА:***\n` +
            `• **🏦 Сбербанк** - классический банковский перевод\n` +
            `• **⚡ СПБ** - мгновенный перевод на Альфа-Банк или Т-Банк\n\n` +
            
            `***💡 СОВЕТЫ:***\n` +
            `• В комментарии к переводу укажите: "${userName} абонемент"\n` +
            `• После перевода сразу отправьте скриншот чека\n` +
...
bot.action(/^bank_sber_(\w+)$/, async (ctx) => {
    try {
        ...
        await ctx.reply(
            `***🏦 РЕКВИЗИТЫ СБЕРБАНК***\n\n` +
            `***📋 ВАШ ЗАКАЗ:***\n` +
            `└─ Тип: ${payment.subscriptionType === 'monthly' ? 'Месячный' : 'Разовый'}\n` +
            ...
            `5. В комментарии укажите: "${payment.userName} абонемент"\n` +
...
bot.action(/^bank_spb_(\w+)$/, async (ctx) => {
    try {
        ...
        await ctx.reply(
            ...
            `**По номеру телефона:**\n` +
            `1. В приложении банка выберите "Перевод по номеру"\n` +
    ...
    try {
        ...
        if (!isTrainingDay) {
            ...
            await ctx.reply(
                ...
                `└─ ⏰ 20:00\n\n` +
                `***💡 РЕКОМЕНДАЦИЯ:***\n` +
                `Запишитесь на ближайшую тренировку заранее!`,
                { format: 'markdown' }
            );
            await ctx.deleteMessage();
            return;
...
bot.action(/^admin_view_payment_(\w+)$/, async (ctx) => {
    try {
        ...
        response += `Тип абонемента: ${payment.subscriptionType === 'monthly' ? '📅 Месячный' : '🎫 Разовый'}\n`;
...
bot.action('admin_subs_stats', async (ctx) => {
    try {
        ...
        response += `Цена за занятие (разовый): ***${singleCostPerLesson} руб.***\n`;
...
bot.action(/^admin_delete_user_(\d+)$/, async (ctx) => {
    try {
        ...
        await ctx.reply(
            ...
            `💳 Абонемент: ${sub ? '✅ ' + (sub.type === 'monthly' ? '📅 Месячный' : '🎫 Разовый') : '❌ Нет'}\n\n` +
...
bot.action(/^admin_delete_sub_user_(\d+)$/, async (ctx) => {
    try {
        ...
        const subType = subscription.type === 'monthly' ? '📅 Месячный' : '🎫 Разовый';
        const startDate = new Date(subscription.startDate);
        const endDate = new Date(startDate);
        endDate.setDate(endDate.getDate() + 30);
        const daysLeft = Math.ceil((endDate - new Date()) / (1000 * 60 * 60 * 24));
...
bot.action(/^admin_offer_sub_(\d+)$/, async (ctx) => {
    try {
        ...
        const message = `Здравствуйте, ${user.name}! 👋\n\n` +
            ...
            `🎫 ***Разовый абонемент*** - 1 занятие за 700 руб.\n` +
            `└─ Для пробного посещения\n` +
